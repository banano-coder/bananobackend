...
router.get('/brands', requireAuth, requireRole('admin','manager','vendedor','viewer'), async (req,res,next)=>{
  try{
    const { rows } = await pool.query(`SELECT id_marca, nombre, activo FROM public.marca ORDER BY nombre`);
    res.json({ data: rows });
  }catch(err){ next(err); }
});
...
router.delete('/brands/:id', requireAuth, requireRole('admin','manager'), async (req,res,next)=>{
  ...
  try{
    ...
    if(hasProd.length){ await client.query('ROLLBACK'); return res.status(400).json({message:'No se puede eliminar: hay productos activos de esta marca'}); }

    const { rowCount } = await client.query(`UPDATE public.marca SET activo=false WHERE id_marca=$1`,[id]);
    if(!rowCount){ await client.query('ROLLBACK'); return res.status(404).json({message:'No encontrada'}); }

    await client.query(`
      INSERT INTO public.auditoria (actor_id, target_tipo, action, payload, created_at)
      VALUES ($1, 'marca', 'BRAND_DISABLE', $2::jsonb, NOW())
    `,[req.user.id || req.user.sub, JSON.stringify({ id_marca: id })]);
